...
##################################################################
#uses the same topic probabilities for all episode
##################################################################
class SensemakingTrainingEnv2(Env):
    def __init__(self, number_clues, number_topics, number_states, start_state, epsilon, maxPostPerStep):
    ...
    def step(self, action):
        ...
        # count_selected = 0
        # for i in range(0,self.number_clues):
        #     #if user has already selected 4 postIts, break
        #     max_selected = random.randint(1,self.maxPostPerStep)
        #     if(count_selected >= max_selected):
        #         break
        #
